...
class LedgerTransactionService {
	...
	public async createTransactionWithEntries(input: CreateTransactionInput): Promise<unknown> {
	...
	public async createSettlement(
		...
		const settledAccount = await this.ledgerRepo.getAccountBalance(settledAccountId)
		const contraAccount = await this.ledgerRepo.getAccountBalance(contraAccountId)
		...
		return await this.createTransactionWithEntries({
			...
			description: description || `Settlement: ${settledAccountId} to ${contraAccountId}`,
			entries,
	...
	public async getTransactionHistory(accountId: string, limit = 50, offset = 0): Promise<unknown[]> {
