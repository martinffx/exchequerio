...
class LedgerAccountEntity {
	...
	public static fromRecord(record: LedgerAccountRecord): LedgerAccountEntity {
		return new LedgerAccountEntity({
			...
			balanceAmount:
				record.balanceAmount === "0.0000" ? "0" : record.balanceAmount,
	...
	public toResponse(): LedgerAccountResponse {
		if (!this.balanceData) {
			throw new Error(
				"Balance data required for response conversion. Use fromRecordWithBalances()",
			);
		}
