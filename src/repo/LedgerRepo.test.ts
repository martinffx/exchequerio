...
describe("LedgerRepo Integration Tests", () => {
  ...
  describe("Balance Calculation Performance", () => {
    it("should calculate balances efficiently for multiple transactions", async () => {
      ...
      const balances = await ledgerRepo.getAccountBalances(
        testAccount1Id,
        testLedgerId,
      );
    ...
    it("should handle fast balance queries for high frequency operations", async () => {
      ...
      const startTime = performance.now();
      const balances = await ledgerRepo.getAccountBalancesFast(
        testAccount1Id,
        testLedgerId,
      );
      const endTime = performance.now();
      ...
      await expect(
        db.insert(LedgerTransactionsTable).values(
          ({
            ...
            typeof db.insert<typeof LedgerTransactionsTable>
          >[0]["values"]) > [0],
        ),
      ).rejects.toThrow();
    });
