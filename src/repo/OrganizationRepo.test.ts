...
describe("OrganizationRepo", () => {
	...
	it("should not allow creating of duplicate records", async () => {
		...
		const rs = await repo.createOrganization(record);
		await expect(repo.createOrganization(record)).rejects.toThrow(
			ConflictError,
		);
		await repo.deleteOrganization(rs.id);
	});
	...
	it("should allow records to be updated", async () => {
		...
		expect(repo.updateOrganization(record.id, record)).rejects.toThrow(
			NotFoundError,
		);
		...
		try {
			...
			const rs1 = await repo.updateOrganization(
				updatedRecord.id,
				updatedRecord,
			);
	...
	it("should paginate records", async () => {
		...
		try {
			...
			await Promise.all(
				savedRecords.map((record) => repo.deleteOrganization(record.id)),
			);
		}
	});
});
