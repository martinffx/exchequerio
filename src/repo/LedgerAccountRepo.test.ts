...
describe("LedgerAccountRepo Integration Tests", () => {
	...
	describe("listLedgerAccounts", () => {
		...
		it("should list all accounts for a ledger with pagination", async () => {
			...
			const _testAccount = await ledgerAccountRepo.createLedgerAccount(testOrgId, entity1);
		});
		...
		it("should delete account when no dependencies exist", async () => {
			...
			await expect(
				ledgerAccountRepo.getLedgerAccount(testOrgId, testLedgerId, testAccountId1)
		...
		it("should throw error when account has transaction entries", async () => {
			...
			await expect(
				ledgerAccountRepo.deleteLedgerAccount(testOrgId, testLedgerId, testAccountId1)
		...
		it("should prevent deletion of account from different ledger", async () => {
			...
			await expect(
				ledgerAccountRepo.deleteLedgerAccount(testOrgId, otherLedgerId, testAccountId1)
	...
	describe("calculateBalance", () => {
		...
		beforeEach(async () => {
			...
			_debitAccount = await ledgerAccountRepo.createLedgerAccount(testOrgId, debitEntity);
			_creditAccount = await ledgerAccountRepo.createLedgerAccount(testOrgId, creditEntity);
		});
	...
	describe("Concurrency and Race Conditions", () => {
		it("should handle concurrent account operations safely", async () => {
			...
			const createPromises = Array.from({ length: 5 }, (_, index) => {
				...
				return ledgerAccountRepo.createLedgerAccount(testOrgId, entity);
			});

			const accounts = await Promise.all(createPromises);

			// All should have unique IDs
			const ids = accounts.map((a: LedgerAccountEntity) => a.id.toString());
		...
		it("should handle concurrent updates with optimistic locking", async () => {
			...
			const baseAccount = await ledgerAccountRepo.createLedgerAccount(testOrgId, entity);
			...
			const updatePromises = Array.from({ length: 3 }, (_, index) => {
				...
				return ledgerAccountRepo.updateLedgerAccount(testOrgId, testLedgerId, updated);
			});
	...
	describe("Error Handling", () => {
		it("should handle database connection errors gracefully", async () => {
			...
			await expect(temporaryRepo.createLedgerAccount(testOrgId, entity)).rejects.toThrow();
		...
		it("should validate TypeID format", async () => {
			...
			await expect(
				ledgerAccountRepo.getLedgerAccount(testOrgId, testLedgerId, invalidId)
			).rejects.toThrow();
		});
	});
	...
	describe("getLedgerAccount", () => {
		...
		it("should return account when found", async () => {
			...
			await ledgerAccountRepo.createLedgerAccount(testOrgId, entity);
		...
		it("should list accounts with organization tenancy", async () => {
			...
			await ledgerAccountRepo.createLedgerAccount(testOrgId, entity1);
	...
	describe("updateLedgerAccount", () => {
		...
		beforeEach(async () => {
			...
			existingAccount = await ledgerAccountRepo.createLedgerAccount(testOrgId, entity);
		});
	...
	describe("deleteLedgerAccount", () => {
		...
		beforeEach(async () => {
			...
			_testAccount = await ledgerAccountRepo.createLedgerAccount(testOrgId, entity);
		});
