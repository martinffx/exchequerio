...
class LedgerTransactionRepo {
  ...
  public async listLedgerTransactions(
    ...
    const allEntries = await this.db
      ...
      .from(LedgerTransactionEntriesTable)
      .where(
        inArray(LedgerTransactionEntriesTable.transactionId, transactionIds),
      );
    ...
    return records.map((record) => {
      const entries =
        entriesByTransaction.get(record.ledger_transactions.id) ?? [];
      return LedgerTransactionEntity.fromRecord(
        record.ledger_transactions,
        entries,
      );
    ...
    try {
      return await this.db.transaction(async (tx) => {
        ...
        const transactionResult = await tx
          ...
          .returning();
        const createdTransaction = LedgerTransactionEntity.fromRecord(
          transactionResult[0],
          [...transaction.entries],
        );
  ...
  public async postTransaction(
    ...
    const accountIds = postedTransaction.entries.map((e) =>
